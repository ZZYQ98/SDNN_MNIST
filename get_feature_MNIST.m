function [X,T_features] = get_feature_MNIST(weights,layers,network_struct,spike_times,num_img,total_time)
...
Sz=n_featuers; %最后一层卷积层的深度层数
       ...
       for ii=1:num_img
           ...
           MNIST_data=spike_times(:,:,n);
              if n<num_img
                  n=n+1;
              else
                  n=1;
              end    
            st= MNIST_coding( MNIST_data,total_time,num_layers);%其中mnist_data为单一的输入图像 编码为spike_time
